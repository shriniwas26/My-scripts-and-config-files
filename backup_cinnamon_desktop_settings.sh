...
mkdir -p $FOLDER
ROTATE_DAYS=20
zip -rv "$FOLDER/linux_mint_config_$(date +"%Y_%m_%d_%H%M").zip" ~/.atom ~/.config ~/.cinnamon ~/.gconf ~/.icons ~/.themes -x \*spotify* \*chromium* \*google-chrome* \*Virtualbox* \*Cache* \*IndexedDB*
